...
## 🧮 二十一、SQLZOO Lesson 1–8 題整理與概念補充
```sql

# 📘 SQLZOO Lesson 1～8 題重點回顧
> 主題：SUM、COUNT、GROUP BY、HAVING 實務應用

---

🧩 第 1 題：Total world population
SELECT SUM(CAST(population AS BIGINT))
FROM world;

💡 說明：
SUM(population) 加總全世界人口。
因人口總數太大需先 CAST 為 BIGINT 避免整數溢位。

---

🧩 第 2 題：List of continents
SELECT DISTINCT continent
FROM world;

💡 說明：
DISTINCT 去除重複，只列出各洲一次。
可用 GROUP BY continent 達成相同效果。

---

🧩 第 3 題：GDP of Africa
SELECT SUM(CAST(gdp AS BIGINT))
FROM world
WHERE continent = 'Africa';

💡 說明：
先篩出 Africa，再加總 GDP。
仍建議 CAST 成 BIGINT 防止溢位。

---

🧩 第 4 題：Count the big countries
SELECT COUNT(name)
FROM world
WHERE area > 1000000;
...
🧩 第 5 題：Baltic states population
SELECT SUM(CAST(population AS BIGINT))
FROM world
WHERE name IN ('Estonia', 'Latvia', 'Lithuania');

💡 說明：
IN 搭配 SUM() 可針對多個國家彙總統計。

---

🧩 第 6 題：Counting the countries of each continent
SELECT continent, COUNT(name)
FROM world
GROUP BY continent;

💡 說明：
GROUP BY 分組後搭配 COUNT()，可計算每洲的國家數。

---

🧩 第 7 題：Counting big countries in each continent
SELECT continent, COUNT(name)
FROM world
...
💡 說明：
先用 WHERE 篩出人口 ≥ 1,000 萬的國家，再分組統計。
⚠️ 常見錯誤：使用 HAVING 或 MAX(population) 錯把條件放在群組後。

---

🧩 第 8 題：Counting big continents
SELECT continent
FROM world
GROUP BY continent
...
💡 說明：
HAVING 用於分組後的篩選（例如總人口 ≥ 1 億）。
若條件放 WHERE 會報錯，因 SUM() 是聚合函數。

---

# 🧠 WHERE vs HAVING 差異對照表
...
| 篩選對象 | 單筆資料列 | 群組結果 |
| 是否可用聚合函數 | ❌ 不可 | ✅ 可 |
| 範例 | WHERE population > 10000000 | HAVING SUM(population) > 100000000 |
...
-- 找出每個洲內總 GDP 超過 10 兆的洲
SELECT continent, SUM(gdp) AS total_gdp
FROM world
GROUP BY continent
HAVING SUM(gdp) > 10000000000000;

-- 找出每個洲中，國家平均人口超過 2,000 萬的洲
